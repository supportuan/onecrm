...
// //     private isMetaSyncRunning = false;
...
// //         throw new Error(
// //             `Meta API Error in ${methodName}: ${JSON.stringify(errorMessage)}`
// //         );
// //     }

// //     startAutoSync() {
// //         if (this.isAutoSyncStarted) {
// //             console.log('[MetaService AutoSync] Already started. Skipping...');
// //             return;
// //         }
...
// //             if (this.isMetaSyncRunning) {
// //                 console.log(
// //                     '[MetaService AutoSync] Previous sync still running. Skipping...'
// //                 );
// //                 return;
// //             }

// //             this.isMetaSyncRunning = true;

// //             try {
// //                 console.log('[MetaService AutoSync] Started');

// //                 const result = await this.syncRecentLeads();

// //                 console.log('[MetaService AutoSync] Completed:', result);
// //             } catch (error: any) {
// //                 console.error('[MetaService AutoSync] Failed:', error.message);
// //             } finally {
// //                 this.isMetaSyncRunning = false;
// //             }
// //         });

// //         console.log('[MetaService AutoSync] Scheduled every 3 hours');
// //     }
...
// //             throw new Error(metaError?.message || 'Failed to fetch Meta lead forms');
// //         }
// //     }
...
// //                 } catch (formError: any) {
// //                     console.warn(
// //                         `[MetaService.syncRecentLeads] Failed to sync form ${form.id}:`,
// //                         formError.message
// //                     );
// //                     continue;
// //                 }
...
// //         } catch (error: any) {
// //             console.error(
// //                 '[MetaService.getOrCreateMetaLeadSource] Database Error:',
// //                 error.message
// //             );
...
// //                         comment:
// //                             'Duplicate Meta lead received. Existing lead updated with activity log.',
// //                         metadata: {
...
// //         } catch (error: any) {
// //             console.error(
// //                 '[MetaService.processLead] Error processing lead:',
// //                 error.message
// //             );
...
//         if (!token) {
//             throw new Error("META_PAGE_ACCESS_TOKEN is missing. Use Page Access Token, not User/App token.");
//         }

//         return token;
//     }

//     private getAdAccessToken() {
//         const token = process.env.META_ACCESS_TOKEN;
...
//         console.error(`[MetaService.${methodName}]`, metaError || error.message);

//         throw new Error(`Meta API Error in ${methodName}: ${message}`);
//     }
...
//             const response = await axios.get(`${this.baseUrl}/${this.version}/${pageId}`, {
//                 params: {
...
//             return response.data;
//         } catch (error: any) {
//             this.handleApiError(error, "fetchPage");
//         }
//     }

//     async fetchLeadForms(pageId?: string) {
//         try {
//             const finalPageId = pageId || process.env.META_PAGE_ID;
...
//             const response = await axios.get(
//                 `${this.baseUrl}/${this.version}/${finalPageId}/leadgen_forms`,
//                 {
//                     params: {
//                         access_token: this.getPageAccessToken(),
...
//             return response.data;
//         } catch (error: any) {
//             this.handleApiError(error, "fetchLeadForms");
//         }
//     }
...
//             const response = await axios.post(
//                 `${this.baseUrl}/${this.version}/${finalPageId}/subscribed_apps`,
//                 null,
//                 {
//                     params: {
//                         access_token: this.getPageAccessToken(),
...
//     async syncAllHistoricalLeads() {
//         const formsResponse = await this.fetchLeadForms();
//         const forms = formsResponse.data || [];

//         let totalSynced = 0;

//         for (const form of forms) {
//             const leadsResponse = await this.fetchHistoricalLeads(form.id);
...
//     startAutoSync() {
//         if (this.isAutoSyncStarted) return;

//         this.isAutoSyncStarted = true;

//         cron.schedule("0 */3 * * *", async () => {
//             if (this.isMetaSyncRunning) return;

//             this.isMetaSyncRunning = true;

//             try {
//                 const result = await this.syncRecentLeads();
//                 console.log("[Meta Auto Sync Completed]", result);
//             } catch (error: any) {
//                 console.error("[Meta Auto Sync Failed]", error.message);
//             } finally {
//                 this.isMetaSyncRunning = false;
//             }
//         });
...
//         if (!Array.isArray(metaLead.field_data)) return mapped;
...
//         if (!mappedLead.fullName || !mappedLead.phone) {
//             console.warn("[Meta Lead Skipped] Missing fullName or phone", mappedLead);
//             return null;
//         }
...
//                     leadId: existingLead.id,
//                     activityType: ActivityType.NOTE,
//                     comment: "Duplicate Meta lead received.",
//                     metadata: metaLead as any,
//                 },
//             });
...
//                 leadId: lead.id,
//                 activityType: ActivityType.NOTE,
//                 comment: "Lead created from Meta Lead Ads.",
//                 metadata: metaLead as any,
//             },
//         });
...
class MetaService {
    ...
    private handleApiError(error: any, methodName: string): never {
        ...
        throw new Error(
            metaError?.message || error.response?.data || error.message
        );
    }
    ...
    async subscribePage(pageId?: string) {
        try {
            const finalPageId = pageId || process.env.META_PAGE_ID;
            if (!finalPageId) throw new Error("META_PAGE_ID is missing");
    ...
    startAutoSync() {
        ...
        cron.schedule("0 * * * *", async () => {
            ...
            try {
                console.log("[Meta Auto Sync] Started");
                const result = await this.syncRecentLeads();
                console.log("[Meta Auto Sync] Completed:", result);
            } catch (error: any) {
                console.error("[Meta Auto Sync] Failed:", error.message);
        ...
        console.log("[Meta Auto Sync] Scheduled every 1 hour");
    }
    ...
    async processLead(metaLead: MetaLeadData) {
        ...
        if (!mappedLead.fullName || !mappedLead.phone) {
            console.warn("[Meta Lead Skipped] Missing name or phone", mappedLead);
            return null;
        }
