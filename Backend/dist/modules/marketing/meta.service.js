...
class MetaService {
    ...
    async fetchHistoricalLeads(formId) {
        try {
    ...
    async fetchUserPages() {
        try {
            ...
            console.error('[MetaService.fetchUserPages] Meta API Error:', metaError?.message || error.message);
    ...
    async fetchLeadForms(pageId) {
        try {
            ...
            const metaError = error.response?.data?.error;
            console.error('[MetaService.fetchLeadForms] Meta API Error:', metaError?.message || error.message);
            throw new Error(metaError?.message || 'Failed to fetch Meta lead forms');
        }
    }
    async subscribePage(pageId) {
        try {
            const response = await axios.post(`${this.baseUrl}/${this.version}/${pageId}/subscribed_apps`, null, {
                params: {
                    access_token: this.getAccessToken(),
    ...
    async syncRecentLeads() {
        try {
            ...
            for (const form of forms) {
                try {
                    const leadsResponse = await axios.get(`${this.baseUrl}/${this.version}/${form.id}/leads`, {
                        params: {
                            access_token: this.getAccessToken(),
                            since: fourHoursAgo,
                            limit: 100,
    ...
    async getOrCreateMetaLeadSource() {
        try {
    ...
    calculateLeadScore(mappedLead, metaLead) {
        let score = 0;
        ...
        // Interest quality
        if (mappedLead.preferredCountry)
            score += 15;
        if (mappedLead.preferredCourse)
            score += 15;
        // Location quality
        if (mappedLead.country)
            score += 10;
