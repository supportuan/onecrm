...
export const assignAccessRole = async (employeeId, role) => {
...
export const createEmployee = async (data) => {
    ...
    const exists = employees.some(e => e.email.toLowerCase() === data.email.toLowerCase() || e.employeeId === data.employeeId);
    if (exists)
...
export const updateEmployee = async (employeeId, updates) => {
    ...
    if (updates.location !== undefined)
        emp.location = updates.location;
    if (updates.employmentStatus !== undefined)
        emp.employmentStatus = updates.employmentStatus;
    if (updates.dob !== undefined)
        emp.dob = updates.dob;
    if (updates.address !== undefined)
        emp.address = updates.address;
    if (updates.emergencyContact !== undefined)
        emp.emergencyContact = updates.emergencyContact;
    if (updates.hireDate !== undefined)
        emp.hireDate = updates.hireDate;
    if (updates.access_role !== undefined)
        emp.access_role = updates.access_role;
    return emp;
};
...
export const uploadEmployeeDocument = async (employeeId, doc) => {
    const emp = employees.find(e => e.id === employeeId || e.employeeId === employeeId);
    if (!emp)
        throw new Error('Employee not found');
...
export const submitRemoteClockIn = async (employeeId, data) => {
    if (!employeeId?.trim()) {
    ...
    if (!emp)
        throw new Error('Employee not found for clock-in');
...
export const processRegularization = async (id, status, remarks) => {
...
export const applyLeaveRequest = async (employeeId, data) => {
    if (!employeeId?.trim())
        throw new Error('Employee ID is required');
...
export const updateCandidate = async (id, updates) => {
    ...
    if (updates.currentStage)
        cand.currentStage = updates.currentStage;
    if (updates.fields)
        cand.fields = { ...cand.fields, ...updates.fields };
    return cand;
};
...
export const createHoliday = async (hol) => {
    const newHol = {
...
export const calculatePayroll = async (month, year) => {
    ...
    for (const struct of salaryStructures) {
        ...
        if (!exists) {
            ...
            const leavesInMonth = leaveRequests.filter(r => r.employeeId === emp.id &&
                r.status === 'APPROVED' &&
            ...
            const attendanceForMonth = attendanceRecords.filter(r => r.employee_id === emp.id &&
                r.status === 'ABSENT' &&
...
export const createBusinessGoal = async (data) => {
    const goal = {
...
export const linkBusinessGoalToHr = async (goalId) => {
    ...
    if (!goal.linkedModules.includes('HR'))
        goal.linkedModules.push('HR');
    return goal;
};
...
export const getAttendanceTemplate = async (id) => {
    ...
    if (!tpl)
        throw new Error('Attendance process template not found');
    return tpl;
};
// ----------------------------------------------------
...
// ----------------------------------------------------
export const getCounselorMetrics = async (counselorId) => {
...
export const updatePerformanceReview = async (id, updates) => {
    ...
    if (updates.kpiScores !== undefined)
        review.kpiScores = updates.kpiScores;
