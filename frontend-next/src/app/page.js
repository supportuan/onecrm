...
const workspaceCards = [
  ...
  },
  {
    ...
    id: "AUN-MKTG",
    icon: Sparkles,
    allowedRoles: ["SUPER_ADMIN", "ADMIN", "COUNSELLOR"],
    accent: "violet",
  },
];
...
export default function Home() {
  ...
  const workspaceStatus = useMemo(
    () =>
      workspaceCards.map((item) => ({
        ...item,
        permitted: Boolean(user && item.allowedRoles.includes(user.role)),
      })),
    [user],
  );
  ...
  const handleConnect = async (workspace) => {
    ...
    const card = workspaceStatus.find((item) => item.workspace === workspace);
      ...
      <div className="relative z-10 w-full max-w-4xl flex flex-col items-center">
        ...
        <div className="flex flex-col items-center text-center mb-12">
          ...
          <p className="mt-3 text-slate-400 text-sm font-semibold flex items-center gap-2">
            <Lock className="h-3.5 w-3.5 text-emerald-400" /> SSO Single Sign-On
            Portal
          </p>
          <div className="mt-4 rounded-3xl border border-slate-800 bg-slate-950/70 px-4 py-3 text-sm text-slate-300 max-w-2xl">
            {isAuthenticated ? (
              <span>
                Logged in as{" "}
                ...
                ({user.role}). Select your available workspace below.
              </span>
            ) : (
              <span>
                Please sign in first to reveal workspace access.{" "}
                <button
            ...
            <div className="w-full max-w-md bg-slate-900 border border-slate-850 rounded-3xl p-8 text-center shadow-2xl relative overflow-hidden">
              ...
              <div className="space-y-4 mt-6 text-left max-w-xs mx-auto">
                <div className="flex items-center gap-3">
                  <span
                    className={`h-2 w-2 rounded-full ${ssoStep >= 1 ? "bg-cyan-400 shadow-sm shadow-cyan-400" : "bg-slate-700"}`}
                  />
                  <span
                    className={`text-xs font-semibold ${ssoStep >= 1 ? "text-cyan-300 font-bold" : "text-slate-500"}`}
                  >
                ...
                <div className="flex items-center gap-3">
                  <span
                    className={`h-2 w-2 rounded-full ${ssoStep >= 2 ? "bg-indigo-400 shadow-sm shadow-indigo-400" : "bg-slate-700"}`}
                  />
                  <span
                    className={`text-xs font-semibold ${ssoStep >= 2 ? "text-indigo-300 font-bold" : "text-slate-500"}`}
                  >
                ...
                <div className="flex items-center gap-3">
                  <span
                    className={`h-2 w-2 rounded-full ${ssoStep >= 3 ? "bg-emerald-400 shadow-sm shadow-emerald-400 animate-pulse" : "bg-slate-700"}`}
                  />
                  <span
                    className={`text-xs font-semibold ${ssoStep >= 3 ? "text-emerald-300 font-bold" : "text-slate-500"}`}
                  >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 w-full">
          {workspaceStatus.map((item) => {
            const allowed = item.permitted;
            const cardClass = allowed
              ? `group relative rounded-3xl bg-slate-900/40 border border-slate-900 ${item.accent === "cyan" ? "hover:border-cyan-500/30" : "hover:border-violet-500/30"}`
              : "group relative rounded-3xl bg-slate-950/30 border border-slate-900";
            const buttonClass = allowed
              ? `flex items-center gap-2 rounded-full ${item.accent === "cyan" ? "bg-cyan-500/10 hover:bg-cyan-500 text-cyan-300 hover:text-slate-950 shadow-cyan-500/20" : "bg-violet-500/10 hover:bg-violet-500 text-violet-300 hover:text-slate-950 shadow-violet-500/20"} font-semibold text-xs px-5 py-2.5 transition-all shadow-lg`
              : "flex items-center gap-2 rounded-full bg-slate-700 text-slate-400 font-semibold text-xs px-5 py-2.5 cursor-not-allowed";
            const iconBgClass =
              item.accent === "cyan"
            ...
            return (
              <div
                key={item.workspace}
                className={`${cardClass} p-8 flex flex-col justify-between transition-all duration-300 ${allowed ? "hover:bg-slate-900/60 hover:-translate-y-1 shadow-xl" : "opacity-60"}`}
              >
                <div>
                  <div
                    className={`flex h-12 w-12 items-center justify-center rounded-2xl ${iconBgClass} mb-6 group-hover:scale-110 transition-transform duration-300`}
                  >
                ...
                <div className="mt-8 pt-6 border-t border-slate-850 flex items-center justify-between">
                  ...
                  <button
                    onClick={() => handleConnect(item.workspace)}
                    disabled={!allowed || !isAuthenticated || isLoggingIn}
                  ...
                  >
                    ...
                    {allowed && <ArrowRight className="h-4 w-4" />}
                  </button>
                </div>
              </div>
            );
        ...
        <div className="mt-20 flex flex-col items-center gap-3 text-center text-xs text-slate-500">
          <div className="flex items-center gap-2 rounded-full bg-slate-900 border border-slate-850 px-4 py-1.5">
            ...
            <span className="font-semibold text-slate-400">
              Authenticated Session Key Exchange Active
            </span>
          </div>
