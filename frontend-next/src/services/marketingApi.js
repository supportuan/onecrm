...
export const launchCampaign = async (campaignId, payload = {}) => {
  const res = await authFetch(`${API_URL}/campaigns/${campaignId}/launch`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
...
export const handleLaunchCampaign = async (campaignId) => {
  ...
  try {
    ...
    console.log("Launch response in component:", response);
...
export const getMarketingPerformanceApi = async (period = "30days") => {
  const res = await authFetch(
    `${API_URL}/analytics/performance?period=${period}`,
  );
  return res.json();
};
...
export const getAgencyFunnelApi = async (period = "30days") => {
  const res = await authFetch(
    `${API_URL}/analytics/agency-funnel?period=${period}`,
  );
  return res.json();
};
...
export const createStudentLogin = async (leadId, password = null) => {
  ...
  const res = await authFetch(
    `${API_URL}/leads/${leadId}/create-student-login`,
    {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(payload),
