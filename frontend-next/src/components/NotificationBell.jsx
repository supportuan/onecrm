  ...
  return (
    <div className="relative" ref={containerRef}>
      <button
        type="button"
        onClick={() => setOpen((o) => !o)}
        className="relative text-slate-400 hover:text-slate-700 transition"
      ...
      {open && (
        <div className="absolute right-0 mt-3 w-[380px] max-h-[540px] bg-white border border-slate-200 rounded-3xl shadow-2xl z-50 flex flex-col overflow-hidden">
          <div className="px-5 py-4 border-b border-slate-200 flex items-center justify-between">
            <div>
              <h3 className="text-xs font-semibold text-slate-800 lowercase">notifications</h3>
          ...
          <div className="flex-1 overflow-y-auto">
            {loading ? (
              <div className="p-10 text-center text-[11px] text-slate-400">loading...</div>
            ) : items.length === 0 ? (
              ...
              <ul className="divide-y divide-slate-100">
                {items.map((n) => {
                  ...
                  return (
                    <li
                      key={n.id}
                      className={`group p-4 hover:bg-slate-50 transition cursor-pointer flex items-start gap-3 ${
                      ...
                      <div className="mt-1.5">
                        <span
                          className={`block w-2 h-2 rounded-full ${
                            isRead ? 'bg-slate-300' : 'bg-indigo-600'
                          }`}
                        />
                      </div>
                      <div className="flex-1 min-w-0">
                        <p className={`text-xs ${isRead ? 'font-medium text-slate-600' : 'font-semibold text-slate-900'} truncate`}>
                          {n.title}
                        </p>
                        <p className="text-[10px] text-slate-500 mt-0.5 line-clamp-2">{n.body}</p>
                        <p className="text-[9px] text-slate-400 mt-1.5">{formatRelative(n.createdAt)}</p>
                      </div>
                      <button
                        onClick={(e) => {
                          e.stopPropagation();
          ...
          <div className="px-5 py-3 border-t border-slate-200 bg-slate-50">
            <a
              href="/notifications"
              className="block text-center text-[10px] font-semibold text-indigo-600 hover:text-indigo-700"
            >
