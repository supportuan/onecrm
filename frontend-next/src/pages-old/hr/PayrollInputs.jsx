  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-800 p-8">
      ...
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        {errorMsg && (
          <div className="p-4 bg-red-50 border border-red-250 rounded-2xl flex items-center gap-3 text-red-700 animate-in fade-in duration-200">
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-left border-collapse">
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {loading ? (
                      <tr>
                        <td colSpan={7} className="px-6 py-12 text-center text-slate-450 flex items-center justify-center gap-3">
                      ...
                      filteredStructures.map((struct) => {
                        ...
                        return (
                          <tr key={struct.id} className="hover:bg-slate-50/50 transition-all duration-200">
                            <td className="px-6 py-5 text-xs font-mono font-semibold text-indigo-600">
                              {struct.employeeId}
                            </td>
                            <td className="px-6 py-5">
                              <div>
                                <p className="text-xs font-semibold text-slate-800">{struct.name}</p>
                                <p className="text-[10px] text-slate-500 mt-0.5">{struct.email}</p>
                              </div>
                            </td>
                            ...
                            <td className="px-6 py-5 text-right">
                              <button 
                                ...
                                className="p-2 bg-white border border-slate-200 hover:border-indigo-600 text-slate-400 hover:text-indigo-650 rounded-xl transition-all"
                              >
                                <Edit2 size={13} />
                              </button>
                            </td>
                      ...
                      <tr>
                        <td colSpan={7} className="px-6 py-12 text-center text-xs text-slate-450 italic">
        ...
        {activeTab === 'execute' && (
          <div className="space-y-8 animate-in fade-in duration-300">
            ...
            <div className="bg-white border border-slate-200 rounded-3xl p-8 shadow-sm flex flex-col lg:flex-row lg:items-center justify-between gap-8">
              <div className="space-y-2">
                <h3 className="text-lg font-semibold text-slate-900">Launch Calculation Batch</h3>
              ...
              <form onSubmit={handleRunPayrollBatch} className="flex flex-col sm:flex-row items-stretch sm:items-center gap-4 bg-slate-50 border border-slate-200 p-2.5 rounded-2xl shrink-0">
                <select 
                  value={executionMonth} 
                  onChange={(e) => setExecutionMonth(parseInt(e.target.value))} 
                  className="bg-transparent text-xs font-semibold px-4 py-2 outline-none cursor-pointer text-slate-800 border-none appearance-none"
                >
                ...
                <button 
                  type="submit"
                  disabled={submitting} 
                  className="bg-indigo-600 text-white px-6 py-3 rounded-xl font-semibold text-[10px] flex items-center justify-center gap-2 hover:bg-indigo-700 transition-all disabled:opacity-50"
                >
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div className="p-8 bg-white border border-slate-200 rounded-3xl flex flex-col gap-4 shadow-sm relative overflow-hidden group hover:scale-[1.01] transition-all">
                <div className="flex items-center justify-between">
                  <span className="text-[10px] font-semibold text-slate-500">Monthly Net Payouts</span>
                  <TrendingUp size={20} className="text-indigo-600 opacity-60" />
                </div>
                <p className="text-3xl font-semibold text-slate-900 tracking-tight">₹{totalPayout.toLocaleString('en-IN')}</p>
                <div className="text-[9px] font-semibold text-slate-400 mt-1">Disbursed Funds Liquidated</div>
              </div>

              <div className="p-8 bg-white border border-slate-200 rounded-3xl flex flex-col gap-4 shadow-sm relative overflow-hidden group hover:scale-[1.01] transition-all">
                <div className="flex items-center justify-between">
                  <span className="text-[10px] font-semibold text-slate-500">Batch LOP Deductions</span>
                  <TrendingDown size={20} className="text-red-500 opacity-60" />
                </div>
                <p className="text-3xl font-semibold text-slate-900 tracking-tight">₹{totalDeductions.toLocaleString('en-IN')}</p>
                <div className="text-[9px] font-semibold text-slate-400 mt-1">Tax withheld / Unpaid Leaves</div>
              </div>

              <div className="p-8 bg-white border border-slate-200 rounded-3xl flex flex-col gap-4 shadow-sm relative overflow-hidden group hover:scale-[1.01] transition-all">
                <div className="flex items-center justify-between">
                  <span className="text-[10px] font-semibold text-slate-500">Disbursed Statements</span>
                  <Users size={20} className="text-emerald-500 opacity-60" />
                </div>
                <p className="text-3xl font-semibold text-slate-900 tracking-tight">{payslips.length}</p>
                <div className="text-[9px] font-semibold text-slate-400 mt-1">Personnel Entries Mapped</div>
              </div>
            </div>

            {/* active entries in executing cycle */}
            <div className="bg-white border border-slate-200 rounded-3xl p-8 shadow-sm">
              <h3 className="text-xs font-semibold text-slate-600 border-b border-slate-200 pb-4 mb-6">Eligible Batches Log</h3>
              <div className="text-center py-10 opacity-40">
                <CheckCircle2 size={36} className="text-emerald-500 mx-auto animate-pulse" />
                <p className="text-xs font-semibold text-slate-800 mt-3">Ledger Engine Live</p>
                <p className="text-[9px] font-medium text-slate-500 max-w-xs mx-auto mt-1">Run an execution batch using the parameters panel above to calculate payouts instantly.</p>
              </div>
            </div>
          </div>
        )}
            ...
            <div className="bg-white border border-slate-200 rounded-3xl p-6 shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-6">
              <div className="relative flex-1">
                ...
                <input
                  ...
                  className="w-full pl-12 pr-4 py-3.5 bg-slate-50 border border-slate-200 rounded-2xl text-xs font-medium text-slate-850 placeholder-slate-400 focus:border-indigo-600 outline-none transition-all"
                  value={searchQuery}
                  onChange={(e) => setSearchQuery(e.target.value)}
                />
              </div>
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-left border-collapse">
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {loading ? (
                      <tr>
                        <td colSpan={8} className="px-6 py-12 text-center text-slate-450 flex items-center justify-center gap-3">
                          <Loader2 className="animate-spin text-indigo-600" size={24} />
                      ...
                      filteredPayslips.map((pay) => (
                        <tr key={pay.id} className="hover:bg-slate-50 transition-all duration-200">
                          <td className="px-6 py-5 text-xs font-mono font-semibold text-indigo-600">
                            {pay.employeeId}
                          </td>
                          <td className="px-6 py-5">
                            <span className="text-xs font-semibold text-slate-800">{pay.name}</span>
                          </td>
                          <td className="px-6 py-5 text-xs font-semibold text-slate-600">
                          ...
                          <td className="px-6 py-5 text-right">
                            <button 
                              ...
                              className="p-2 bg-white border border-slate-200 hover:border-indigo-600 text-slate-600 hover:text-indigo-600 rounded-xl transition-all inline-flex items-center gap-1.5 text-[9px] font-semibold"
                            >
                              <Eye size={13} /> View
                            </button>
                          </td>
                      ...
                      <tr>
                        <td colSpan={8} className="px-6 py-12 text-center text-xs text-slate-500 italic">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-white border border-slate-200 rounded-3xl w-full max-w-md overflow-hidden shadow-2xl scale-100 animate-in zoom-in-95 duration-200">
            <div className="px-6 py-4 border-b border-slate-200 flex justify-between items-center bg-slate-50">
              <h2 className="text-xs font-semibold text-slate-800">Override Salary Ledger</h2>
              <button 
            ...
            <form onSubmit={handleUpdateStructureSubmit} className="p-8 space-y-5">
              <p className="text-[10px] font-semibold text-slate-500 border-b border-slate-200 pb-2 mb-4 leading-none">
                {editingStruct.name} · {editingStruct.employeeId}
              </p>

              <div className="space-y-2">
                <label className="text-[10px] font-semibold text-slate-600 ml-1">Basic Base Salary</label>
                <input
                  type="number" 
                  required
              ...
              <div className="space-y-2">
                <label className="text-[10px] font-semibold text-slate-600 ml-1">Variable Allowances</label>
                <input
                  type="number" 
                  required
              ...
              <div className="space-y-2">
                <label className="text-[10px] font-semibold text-slate-600 ml-1">Mandatory Withholding / Deductions</label>
                <input
                  type="number" 
                  required
                  value={editingStruct.deductions}
                  onChange={e => setEditingStruct({ ...editingStruct, deductions: parseFloat(e.target.value) || 0 })}
                  className="w-full px-5 py-3.5 bg-slate-50 border border-slate-200 rounded-2xl text-xs font-semibold text-red-655 focus:border-indigo-600 outline-none transition-all"
                />
              </div>
              ...
              <div className="flex gap-3 border-t border-slate-200 pt-6 mt-6">
                ...
                <button
                  type="submit"
                  disabled={submitting}
                  className="flex-1 py-3.5 bg-indigo-600 text-white rounded-2xl font-semibold text-[10px] hover:bg-indigo-700 transition-all disabled:opacity-50 flex items-center justify-center gap-1.5"
                >
      ...
      {selectedPayslip && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-250">
          <div className="bg-white border border-slate-200 rounded-3xl w-full max-w-2xl overflow-hidden shadow-2xl scale-100 animate-in zoom-in-95 duration-250">
            {/* Sheet header */}
            <div className="px-8 py-5 border-b border-slate-200 bg-slate-50 flex justify-between items-center">
              <div className="flex items-center gap-2 text-indigo-600 font-semibold text-[10px]">
            ...
            {/* Statement Sheet */}
            <div className="p-8 space-y-8 bg-white text-slate-800" id="payslip-sheet">
              {/* Institution and Period */}
              <div className="flex justify-between items-start border-b border-slate-200 pb-6">
                <div>
                  <h3 className="text-sm font-semibold tracking-[0.2em] text-indigo-600 leading-none">UNIVERSITY INSTITUTION</h3>
                  <p className="text-[9px] font-semibold text-slate-500 mt-1.5">Official Monthly Statement</p>
                </div>
                <div className="text-right">
                  <p className="text-xs font-semibold text-slate-800">{MONTHS[selectedPayslip.month - 1]} {selectedPayslip.year}</p>
                  <p className="text-[9px] text-slate-500 font-semibold mt-0.5">Reference ID: {selectedPayslip.id}</p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4 text-xs">
                <div>
                  <span className="block text-[9px] font-semibold text-slate-500">Beneficiary Name</span>
                  <span className="font-semibold text-slate-800">{selectedPayslip.name}</span>
                </div>
                <div className="text-right">
                  <span className="block text-[9px] font-semibold text-slate-500">University ID</span>
                  <span className="font-mono font-semibold text-indigo-600">{selectedPayslip.employeeId}</span>
                </div>
              </div>
              ...
              <div className="flex justify-between items-center bg-slate-50 border border-slate-200 p-5 rounded-2xl">
                <div>
                  <span className="block text-[9px] font-semibold text-slate-600 tracking-[0.2em] leading-none mb-1">Total Net Liquidity Disbursed</span>
                  <span className="text-[9px] font-medium text-slate-500 italic">Transferred to enrolled financial nodes</span>
                </div>
            ...
            <div className="px-8 py-5 border-t border-slate-200 bg-slate-50 flex justify-end gap-3">
              <button 
                onClick={() => setSelectedPayslip(null)}
                className="px-5 py-3 border border-slate-200 rounded-2xl text-[10px] font-semibold hover:bg-slate-100 text-slate-600 transition-all"
              >
                Close
              </button>
              <button 
                ...
                className="px-6 py-3 bg-indigo-600 text-white rounded-2xl font-semibold text-[10px] shadow-sm hover:bg-indigo-700 transition-all flex items-center gap-1.5"
              >
                <Download size={13} />
