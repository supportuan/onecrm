          ...
          <div className="bg-white rounded-xl shadow-xl w-full max-w-lg p-6 max-h-[90vh] overflow-y-auto">
            ...
            <div className="flex gap-3 mt-6">
              ...
              <button onClick={handleSchedule} disabled={submitting}
                className="flex-1 bg-violet-600 text-white rounded-lg py-2 text-sm font-medium hover:bg-violet-700 disabled:opacity-50 flex items-center justify-center gap-2">
                ...
                <div className="grid grid-cols-2 gap-2">
                  {Object.entries(RECOMMENDATION_META).map(([k, v]) => (
                    <button key={k} onClick={() => setFeedback(f => ({ ...f, recommendation: k }))}
                      className={`px-3 py-2 rounded-lg text-xs font-medium border transition-colors ${feedback.recommendation === k ? 'border-violet-400 bg-violet-50 text-violet-700' : 'border-gray-200 text-gray-600 hover:bg-gray-50'}`}>
                      {v.label}
                    </button>
                  ))}
                </div>
