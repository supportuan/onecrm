...
const Automation = () => {
  ...
  return (
    <div className="space-y-6">
      {/* FILTER & ACTIONS BAR */}
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between bg-white px-2 py-1.5 rounded-2xl border border-slate-200/50 shadow-xs">
        <div className="flex flex-1 items-center gap-3 rounded-full border border-slate-200 bg-slate-50 px-4 py-2 sm:max-w-md shadow-xs transition focus-within:ring-2 focus-within:ring-[#0084ff]/20 focus-within:border-[#0084ff]/60">
      ...
      {/* ERROR FEEDBACK */}
      {error && (
        <div className="flex items-center gap-3 bg-red-50 border border-red-200 text-red-700 px-5 py-4 rounded-2xl text-sm font-semibold shadow-xs animate-in slide-in-from-top-4">
      ...
      {loading && automations.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-32 bg-white rounded-3xl border border-slate-200 shadow-xs">
          ...
          <p className="text-sm text-slate-400 font-semibold mt-4">Retrieving active marketing automations...</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-6 items-start">

          {/* Active Automation Workflows Table Card (3/5 columns) */}
          <div className="lg:col-span-3 bg-white border border-slate-200/80 rounded-[20px] shadow-2xs overflow-hidden flex flex-col justify-between min-h-[480px]">
            <div>
              <div className="flex items-center justify-between px-6 py-5 border-b border-slate-100">
                <h2 className="text-lg font-extrabold text-[#1a2b4c] tracking-tight">Active Automation Workflows</h2>
                <button
                  onClick={() => setSearch('')}
                  className="border border-slate-200 hover:bg-slate-50 px-4 py-1.5 rounded-lg text-xs font-semibold text-slate-600 flex items-center gap-1.5 transition cursor-pointer shadow-3xs active:scale-95"
                >
                  View All
                  <ChevronRight className="h-3.5 w-3.5 text-slate-500 stroke-[2.5]" />
                </button>
              </div>
              ...
              {automations.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-24 text-center px-6">
                  ...
                  <h3 className="text-lg font-semibold text-slate-700">No automation workflows found</h3>
                  <p className="text-sm text-slate-400 font-medium mt-1">Create a new trigger-based stream or adjust filters to begin.</p>
                </div>
              ) : (
                <div className="overflow-x-auto">
                  <table className="w-full border-collapse text-left">
                    <thead>
                      <tr className="bg-[#f8fafc] border-b border-slate-100 font-semibold text-xs text-[#556987]">
                    ...
                    <tbody className="divide-y divide-slate-100 text-sm font-semibold text-slate-700">
                      {automations.map((flow) => (
                        <tr key={flow.id} className="hover:bg-slate-50/50 transition">
                          {/* Workflow Name */}
                          <td className="px-6 py-4.5">
                            <span className="font-semibold text-slate-900 block leading-tight">{flow.workflowName}</span>
                          </td>
                          {/* Trigger */}
                          <td className="px-5 py-4.5 text-xs text-slate-500 font-semibold max-w-[150px] truncate">
                            {flow.trigger}
                          </td>
                          {/* Action */}
                          <td className="px-5 py-4.5 text-xs text-slate-600 font-semibold max-w-[180px] truncate">
                            {flow.action}
                          </td>
                          {/* Status Badge */}
            ...
            {totalPages > 1 && (
              <div className="flex items-center justify-between border-t border-slate-100 px-6 py-4 bg-[#f8fafc]/50">
                ...
                <div className="flex items-center gap-1.5">
                  <button
                    ...
                    className="p-1.5 border border-slate-200 rounded-lg bg-white hover:bg-slate-50 transition disabled:opacity-50 cursor-pointer disabled:pointer-events-none active:scale-95"
                  >
                    <ChevronLeft className="h-3.5 w-3.5 text-slate-600 stroke-[2.5]" />
                  </button>
                  {Array.from({ length: totalPages }, (_, i) => i + 1).map((p) => (
                    <button
                      key={p}
                      onClick={() => setPage(p)}
                      className={`px-3 py-1.5 text-xs font-bold rounded-lg transition active:scale-95 ${page === p ? 'bg-[#0084ff] text-white font-extrabold shadow-sm' : 'bg-white hover:bg-slate-50 border border-slate-200 text-slate-600'}`}
                    >
                      {p}
                    </button>
                  ))}
                  <button
                    ...
                    className="p-1.5 border border-slate-200 rounded-lg bg-white hover:bg-slate-50 transition disabled:opacity-50 cursor-pointer disabled:pointer-events-none active:scale-95"
                  >
                    <ChevronRight className="h-3.5 w-3.5 text-slate-600 stroke-[2.5]" />
                  </button>
                </div>
              </div>
            )}
          ...
          <div className="lg:col-span-2 bg-white border border-slate-200/80 rounded-[20px] shadow-2xs p-6 flex flex-col justify-between min-h-[480px]">
            <div>
              <h2 className="text-[17px] font-semibold text-[#1a2b4c] tracking-tight border-b border-slate-100 pb-4 mb-2 text-left">
                Automation Performance Summary
              </h2>

              <div className="divide-y divide-slate-100 font-semibold text-[13.5px] text-slate-600">
                <div className="py-4 flex justify-between items-center">
                  <span>Total active Workflows</span>
                  <strong className="text-[15px] text-slate-900 font-semibold">
                ...
                <div className="py-4 flex justify-between items-center border-b border-slate-100">
                  ...
                  <strong className="text-[15.5px] text-[#0084ff] font-semibold">
            ...
            <div className="bg-slate-50 border border-slate-200/50 rounded-xl p-4.5 mt-6 text-xs leading-relaxed text-slate-400 font-semibold flex items-start gap-2.5">
              <Info className="h-4.5 w-4.5 text-[#0084ff] flex-shrink-0 stroke-[2.5]" />
              <span>
      ...
      {isCreateOpen && (
        <div className="fixed inset-0 bg-slate-950/40 backdrop-blur-xs flex items-center justify-center p-4 z-50 animate-in fade-in duration-200">
          <div className="bg-white border border-slate-200 rounded-[24px] shadow-2xl max-w-lg w-full overflow-hidden animate-in zoom-in-95 duration-200 flex flex-col">
            <div className="flex items-center justify-between px-6 py-5 border-b border-slate-100 bg-[#f8fafc]">
              <h3 className="text-lg font-extrabold text-[#1a2b4c]">Create New Automation Stream</h3>
              <button
            ...
            <form onSubmit={handleCreateSubmit} className="p-6 space-y-4 font-semibold text-slate-700 text-sm">
              <div>
                ...
                <input
                  ...
                  className="w-full border border-slate-200 rounded-xl px-4 py-2.5 outline-none focus:ring-2 focus:ring-[#0084ff]/20 focus:border-[#0084ff]"
                />
              </div>

              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-semibold text-slate-400 mb-1.5">Trigger</label>
                  <input
                    type="text"
                    required
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <select
                    ...
                    className="w-full border border-slate-200 rounded-xl px-4 py-2.5 bg-white outline-none focus:ring-2 focus:ring-[#0084ff]/20 focus:border-[#0084ff]"
                  >
                ...
                <div>
                  <label className="block text-xs font-semibold text-slate-400 mb-1.5">Leads Automated</label>
                  <input
                    type="number"
              ...
              <div className="grid grid-cols-3 gap-3">
                <div>
                  <label className="block text-xs font-semibold text-slate-400 mb-1">Emails Sent</label>
                  <input
                    type="number"
                    value={workflowForm.emailsSent}
                    onChange={(e) => setWorkflowForm({ ...workflowForm, emailsSent: e.target.value })}
                    className="w-full border border-slate-200 rounded-xl px-3 py-2 outline-none focus:ring-2 focus:ring-[#0084ff]/20 focus:border-[#0084ff]"
                  />
                </div>
                <div>
                  <label className="block text-xs font-semibold text-slate-400 mb-1">WhatsApp Sent</label>
                  <input
                    type="number"
                ...
                <div>
                  <label className="block text-xs font-semibold text-slate-400 mb-1">SMS Sent</label>
                  <input
                    type="number"
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-400 mb-1.5">Engagement Rate (%)</label>
                <input
                  type="number"
                  step="0.1"
                  max="100"
              ...
              <div className="flex items-center justify-end gap-3 pt-3 border-t border-slate-100">
                <button
                  type="button"
                  onClick={() => setIsCreateOpen(false)}
                  className="border border-slate-200 hover:bg-slate-50 px-5 py-2.5 rounded-xl font-semibold text-slate-700 transition cursor-pointer"
                >
                  Cancel
                </button>
                <button
                  type="submit"
                  className="bg-[#1a2b4c] hover:bg-[#253b66] text-white px-6 py-2.5 rounded-xl font-semibold transition cursor-pointer shadow-md"
                >
                  Create Flow
                </button>
              </div>
      ...
      {isEditOpen && (
        <div className="fixed inset-0 bg-slate-950/40 backdrop-blur-xs flex items-center justify-center p-4 z-50 animate-in fade-in duration-200">
          <div className="bg-white border border-slate-200 rounded-[24px] shadow-2xl max-w-lg w-full overflow-hidden animate-in zoom-in-95 duration-200 flex flex-col">
            <div className="flex items-center justify-between px-6 py-5 border-b border-slate-100 bg-[#f8fafc]">
              <h3 className="text-lg font-extrabold text-[#1a2b4c]">Edit Automation Stream</h3>
              <button
            ...
            <form onSubmit={handleEditSubmit} className="p-6 space-y-4 font-semibold text-slate-700 text-sm">
              <div>
                <label className="block text-xs font-semibold text-slate-400 mb-1.5">Workflow Name</label>
                <input
                  type="text"
