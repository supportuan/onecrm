...
// const formatBudgetValue = (campaign) => {
//   if (campaign?.type === 'EMAIL') return 'N/A';
//   return formatCurrency(campaign?.budget);
// };

// const parseLaunchDetails = (details) => {
//   if (!details) return {};
//   if (typeof details === 'string') {
//     try {
//       return JSON.parse(details);
//     } catch (err) {
//       console.error('Invalid launchDetails JSON:', err);
//       return {};
//     }
//   }
//   return details;
...
// const getDefaultLaunchDetails = (type) => {
//   switch (type) {
//     case 'SOCIAL_MEDIA':
//       return {
//         platform: 'FACEBOOK',
...
//       return { smsContent: '' };
//     case 'WHATSAPP':
//       return { whatsappTemplate: '', whatsappContent: '', ctaText: '', ctaUrl: '' };
//     default:
//       return {};
//   }
// };

// const normalizeLaunchDetails = (type, details) => ({
//   ...getDefaultLaunchDetails(type),
//   ...parseLaunchDetails(details),
// });

// const usesBudget = (type) => type !== 'EMAIL';
...
//   const showBudgetFields = usesBudget(campaignForm.type);
...
//       launchDetails: normalizeLaunchDetails(campaign.type || 'EMAIL', campaign.launchDetails),
//     });
...
//       camp.type === 'EMAIL' ? 'N/A' : camp.budget || 0,
//       camp.type === 'EMAIL' ? 'N/A' : camp.spent || 0,
//       camp.leadsCount || 0,
...
//                             Daily: {formatCurrency(parseLaunchDetails(camp.launchDetails).dailyBudget)}
//                           </span>
//                         )}
//                       </td>
...
//             <form onSubmit={handleCreateSubmit} className="p-6 space-y-8 max-h-[450px] overflow-y-auto bg-neutral-50/50 font-semibold text-xs text-neutral-500">
...
//                         ...p,
//                         type: selectedType,
//                         budget: selectedType === 'EMAIL' ? '' : p.budget,
//                         spent: selectedType === 'EMAIL' ? 0 : p.spent,
//                         audienceType:
//                           selectedType === 'EMAIL' ||
//                             selectedType === 'SMS' ||
...
const Campaigns = () => {
  ...
  const handleOpenEdit = async (campaign) => {
    ...
    try {
      ...
      if (!response?.success) {
        alert(response?.message || 'Failed to load campaign details.');
        return;
      }

      const fullCampaign = response.data || campaign;
      const campaignType = fullCampaign.type || 'EMAIL';
      ...
      setCampaignForm({
        ...
        type: campaignType,
        budget:
          fullCampaign.budget !== null && fullCampaign.budget !== undefined
            ? String(fullCampaign.budget)
            : '',
        spent:
        ...
        audienceType:
          fullCampaign.audienceType || fullCampaign.selectedAudienceType || 'ALL',
        targetAudience:
          fullCampaign.targetAudience || fullCampaign.audienceDescription || '',
        description: fullCampaign.description || '',
        launchDetails: normalizeLaunchDetails(
          campaignType,
          fullCampaign.launchDetails
        ),
      });
  ...
  return (
    <div className="space-y-6">
                        ...
                        {activeMenuId === camp.id && (
                          ...
                          >
                            ...
                            <button
                              type="button"
                              onClick={() => handleOpenEdit(camp)}
                              disabled={loadingEditCampaign}
                              className="w-full px-4 py-2 hover:bg-neutral-50 text-xs font-semibold text-neutral-700 flex items-center gap-2 transition disabled:cursor-not-allowed disabled:opacity-50"
                            >
      ...
      {isEditOpen && selectedCampaign && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/20 backdrop-blur-xs transition p-4 animate-fade-in">
          <div className="w-full max-w-3xl max-h-[500px] bg-white rounded-[24px] border border-neutral-100 shadow-xl flex flex-col overflow-hidden">
            ...
            <form
              onSubmit={handleEditSubmit}
              className="flex-1 overflow-y-auto p-6 space-y-4 bg-neutral-50/50 font-semibold text-xs text-neutral-500"
            >
              <div className="space-y-1">
                <label>Campaign Identity Name *</label>
                <input
